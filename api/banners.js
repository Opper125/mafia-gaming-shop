...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (banners) {
                const validated = ensureArray(banners);
            ...
            if (banner) {
                ...
                let bans = ensureArray(getData.record);
                const idx = bans.findIndex(b => b.id === banner.id);
                
                if (idx !== -1) {
                    bans[idx] = { ...bans[idx], ...banner, updatedAt: new Date().toISOString() };
                } else {
                    bans.push({ 
                        ...banner, 
                        id: banner.id || `banner_${Date.now().toString(36)}`, 
        ...
        if (req.method === 'DELETE') {
            ...
            const getData = await getRes.json();
            const bans = ensureArray(getData.record).filter(b => b.id !== bannerId);
